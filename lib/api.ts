...
export const authApi = {
  ...
  resetPassword: (email: string, password: string) => api.post("/auth/reset-password", { email, password }),
}

// User API functions
export const userApi = {
  getAllUsers: (page = 1, limit = 20) => api.get(`/user/all-user?page=${page}&limit=${limit}`),
...
export const interestApi = {
  createCategory: (name: string) => api.post("/interest/create-category", { name }),

  getCategories: () => api.get("/interest/categories"),

  updateCategory: (id: string, name: string) => api.patch(`/interest/category/${id}`, { name }),
  ...
  updateInterest: (id: string, data: any) => api.post(`/interest/update-interest/${id}`, data),
...
export const badgeApi = {
  createBadge: (data: { name: string; tag: string; info: string; color: string }) => api.post("/badges/", data),

  getBadges: (page = 1, limit = 20) => api.get(`/badges/?page=${page}&limit=${limit}`),

  getSingleBadge: (id: string) => api.get(`/badges/${id}`),

  updateBadge: (id: string, data: { name: string; tag: string; info: string; color: string }) =>
...
export const reportApi = {
  ...
  getReports: (page = 1, limit = 20) => api.get(`/reports/?page=${page}&limit=${limit}`),

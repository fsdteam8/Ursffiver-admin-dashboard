...
export default function InterestsPage() {
  ...
  const getColorClass = (color: string) => {
    const colorMap: Record<string, string> = {
      ...
      green: "bg-green-100 borderníkov-green-200",
  ...
  const handleDeleteInterests = async () => {
    try {
      await Promise.all(selectedInterests.map((id) => interestApi.deleteInterest(id)));
        ...
        {categoriesLoading || interestsLoading ? (
          ...
          categories.map((category: Category) => {
            ...
            return (
              <div
                key={category._id}
                className="space-y-4 border border-[#F2F4F7] rounded-lg shadow-sm"
              >
                <div className="flex items-center justify-between border-b border-[#F2F4F7] p-4">
                ...
                <div className="space-y-3 p-4">
                  {categoryInterests.length > 0 ? (
                    categoryInterests.map((interest: Interest) => (
                      <div
                        key={interest._id}
                        className={`flex items-center space-x-3 p-3 rounded-lg border group ${getColorClass(
                          interest.color
                        )}`}
                      >
                        <Checkbox
                          ...
                          onCheckedChange={() => handleInterestToggle(interest._id)}
                        />
                        <label
                        ...
                        <Button
                          variant="outline"
                          className="cursor-pointer hidden group-hover:block relative"
                          onClick={() => handleEditInterest(interest)}
                          data-tooltip="Edit Interest"
